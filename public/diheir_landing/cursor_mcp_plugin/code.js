...
async function handleCommand(command, params) {
  switch (command) {
    ...
    case "set_image_fill_from_url":
      return await setImageFillFromUrl(params);
    case "set_image_fill_from_base64":
      return await setImageFillFromBase64(params);
    case "set_image_fills_from_urls":
      return await setImageFillsFromUrls(params);
    case "clear_image_url_cache":
      return clearImageUrlCache();
    case "list_available_fonts":
      return await listAvailableFonts(params);
    case "set_text_fonts":
      return await setTextFonts(params);
    case "generate_product_banners":
      return await generateProductBanners(params);
    ...
    case "hide_live_banner_aux_frames":
      return await hideLiveBannerAuxFrames(params);
    case "show_exclusive_badge_frames":
      return await showExclusiveBadgeFrames(params);
    case "apply_necklace_image_gradient":
      return await applyNecklaceImageGradient(params);
    case "delete_option_suffix_banner_frames":
      return await deleteOptionSuffixBannerFrames(params);
    case "update_live_banner_final_prices":
      return await updateLiveBannerFinalPrices(params);
    default:
...
// ---------------------------------------------------------------------------
// Gradient fill (kakao_live fork extension)
//
// Apply a GRADIENT_LINEAR / GRADIENT_RADIAL / GRADIENT_ANGULAR / GRADIENT_DIAMOND
// paint to a node. By default the gradient is APPENDED to the node's existing
// fills (so it layers on top of image fills). Pass `replace: true` to overwrite
// existing fills. Stops accept positions in 0..1 and colors as {r,g,b,a} in 0..1.
// ---------------------------------------------------------------------------
async function setGradientFill(params) {
  ...
  const paint = {
    type,
    gradientStops,
    gradientTransform: Array.isArray(transform) ? transform : defaultTransform,
    blendMode,
  };
  if (opacity !== undefined) paint.opacity = Number(opacity);

  const existing = Array.isArray(node.fills) ? node.fills.slice() : [];
  let newFills;
  if (replace) {
    newFills = [paint];
  } else if (typeof insertAt === "number") {
    newFills = existing.slice();
    newFills.splice(insertAt, 0, paint);
  } else {
    newFills = existing.concat([paint]);
  }
  node.fills = newFills;
  ...
  return {
    ...
    appliedIndex: newFills.indexOf(paint),
  };
}

// ---------------------------------------------------------------------------
// Image fill from URL (kakao_live fork extension)
//
// The plugin runs in Figma Desktop where `fetch` is available as long as the
// target domain is listed in manifest.networkAccess.allowedDomains. We download
// the bytes, cache the resulting Figma image hash per URL, and apply it as an
// IMAGE fill. A tiny 1x1 gray PNG is used as the fallback when the URL fails.
// ---------------------------------------------------------------------------

const __imageUrlHashCache = new Map();
let __fallbackImageHash = null;

function clearImageUrlCache() {
  const size = __imageUrlHashCache.size;
  __imageUrlHashCache.clear();
  return { cleared: size };
}

async function getFallbackImageHash() {
  if (__fallbackImageHash) return __fallbackImageHash;
  const png = new Uint8Array([
    ...
    0x54, 0x08, 0x99, 0x63, 0xa8, 0xa8, 0xa8, 0x00,
    0x00, 0x02, 0x7c, 0x01, 0x7e, 0xcb, 0x5e, 0xe0,
    0x55, 0x00, 0x00, 0x00, 0x00, 0x49, 0x45, 0x4e,
    0x44, 0xae, 0x42, 0x60, 0x82,
  ]);
  const img = figma.createImage(png);
  __fallbackImageHash = img.hash;
  return __fallbackImageHash;
}

async function resolveNodeForImage(params) {
  const { nodeId, pageName, frameName, nodeName } = params || {};
  if (nodeId) {
    const n = await figma.getNodeByIdAsync(nodeId);
    if (!n) throw new Error("Node not found: " + nodeId);
    return n;
  }
  if (!nodeName) throw new Error("Missing nodeId or nodeName");
  ...
  let scope = page;
  if (frameName) {
    const frame = page.findOne((n) => n.name === frameName);
    if (!frame) throw new Error("Frame not found: " + frameName);
    scope = frame;
  }
  const target = scope.findOne((n) => n.name === nodeName);
  if (!target) throw new Error("Node not found by name: " + nodeName);
  return target;
}

async function loadImageFromUrl(url) {
  if (__imageUrlHashCache.has(url)) {
    return { hash: __imageUrlHashCache.get(url), cached: true, via: "cache" };
  }
  let primaryErr = null;
  if (typeof figma.createImageAsync === "function") {
    try {
      const img = await figma.createImageAsync(url);
      __imageUrlHashCache.set(url, img.hash);
      return { hash: img.hash, cached: false, via: "createImageAsync" };
    } catch (e) {
      primaryErr = String(e && e.message ? e.message : e);
    }
  }
  try {
    ...
    if (bytes.length < 8) throw new Error("Empty response");
    const img = figma.createImage(bytes);
    __imageUrlHashCache.set(url, img.hash);
    return { hash: img.hash, cached: false, bytes: bytes.length, via: "fetch" };
  } catch (e) {
    const msg = String(e && e.message ? e.message : e);
    throw new Error(
      primaryErr ? "createImageAsync: " + primaryErr + " | fetch: " + msg : msg
    );
  }
}

async function applyImageFillToNode(node, hash, scaleMode) {
...
async function setImageFillFromBase64(params) {
  const { base64, scaleMode } = params || {};
  if (!base64) throw new Error("Missing base64 parameter");
  const node = await resolveNodeForImage(params);
  const binary = atob(base64);
  const bytes = new Uint8Array(binary.length);
  for (let i = 0; i < binary.length; i++) bytes[i] = binary.charCodeAt(i);
  const img = figma.createImage(bytes);
  await applyImageFillToNode(node, img.hash, scaleMode);
  return {
    id: node.id,
    name: node.name,
    hash: img.hash,
...
async function setImageFillFromUrl(params) {
  const { url, scaleMode, useFallbackOnError } = params || {};
  if (!url) throw new Error("Missing url parameter");
  const node = await resolveNodeForImage(params);
  try {
    const loaded = await loadImageFromUrl(url);
    await applyImageFillToNode(node, loaded.hash, scaleMode);
    return {
      id: node.id,
      name: node.name,
      url,
    ...
    if (useFallbackOnError === false) throw e;
    const fb = await getFallbackImageHash();
    await applyImageFillToNode(node, fb, scaleMode);
    return {
      id: node.id,
      name: node.name,
      url,
...
// ---------------------------------------------------------------------------
// Font handling (kakao_live fork extension)
// ---------------------------------------------------------------------------
...
async function listAvailableFonts(params) {
  ...
  if (family) {
    ...
    const filtered = fonts.filter((x) => x.fontName.family.toLowerCase().includes(f));
    return {
      total: fonts.length,
      filtered: filtered.length,
      fonts: filtered.map((x) => ({ family: x.fontName.family, style: x.fontName.style })),
    };
  }
  const families = Array.from(new Set(fonts.map((x) => x.fontName.family))).sort();
  return { total: fonts.length, families };
}

async function setTextFonts(params) {
  const { items, pageName, frameName } = params || {};
  ...
  for (const it of items) {
    if (!it.family || !it.style) throw new Error("Each item needs family and style");
    needed.add(it.family + "|" + it.style);
  }
  const loadResults = {};
  for (const key of needed) {
    ...
    try {
      ...
      loadResults[key] = "err:" + String(e && e.message ? e.message : e);
    }
  }
  ...
  for (const it of items) {
    try {
      let node = null;
      if (it.nodeId) {
        node = await figma.getNodeByIdAsync(it.nodeId);
      } else if (it.nodeName) {
        const localScope =
          it.frameName && scope.findOne
            ? scope.findOne((n) => n.name === it.frameName) || scope
            : scope;
        node = localScope.findOne((n) => n.type === "TEXT" && n.name === it.nodeName);
      } else if (it.textMatch) {
        const re = new RegExp(it.textMatch);
        node = scope.findOne((n) => n.type === "TEXT" && re.test(n.characters));
      }
      if (!node) {
        results.push({ ok: false, target: it, error: "node not found" });
        continue;
      }
      if (node.type !== "TEXT") {
        results.push({ ok: false, target: it, error: "not a text node" });
        continue;
      }
      if (loadResults[it.family + "|" + it.style] !== "ok") {
        results.push({
          ok: false,
          id: node.id,
          name: node.name,
          error: "font load failed: " + loadResults[it.family + "|" + it.style],
        });
        continue;
      }
      node.fontName = { family: it.family, style: it.style };
      results.push({
        ok: true,
        id: node.id,
        name: node.name,
        characters: node.characters,
        font: { family: it.family, style: it.style },
      });
    } catch (e) {
      results.push({
        ok: false,
...
async function setImageFillsFromUrls(params) {
  const { items, batchSize, scaleMode, useFallbackOnError } = params || {};
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error("items must be a non-empty array");
  }
  const batch = Math.max(1, Math.min(5, Number(batchSize) || 3));
  const results = [];
  for (let i = 0; i < items.length; i += batch) {
    ...
    const settled = await Promise.all(
      slice.map((it) =>
        setImageFillFromUrl({
          ...
          url: it.url,
          scaleMode: it.scaleMode || scaleMode,
          useFallbackOnError:
            it.useFallbackOnError !== undefined
              ? it.useFallbackOnError
              : useFallbackOnError,
        }).catch((e) => ({
          url: it.url,
          status: "error",
    ...
    figma.notify(
      "image sync " + Math.min(i + batch, items.length) + "/" + items.length
    );
  }
  return {
    total: items.length,
    ...
    cacheSize: __imageUrlHashCache.size,
    results,
  };
}

// ---------------------------------------------------------------------------
// Hide auxiliary promo frame inside live product banners (productInfo / Frame 36)
// ---------------------------------------------------------------------------

async function hideLiveBannerAuxFrames(params) {
  const { pageName } = params || {};
  let page = figma.currentPage;
  if (pageName) {
    await figma.loadAllPagesAsync();
    const p = figma.root.children.find((n) => n.name === pageName);
    if (!p) throw new Error("Page not found: " + pageName);
  ...
  return { page: page.name, hiddenCount: hidden.length, nodeIds: hidden };
}

// ---------------------------------------------------------------------------
// Show "Frame 38" (단독 배지) inside banners whose productTitle contains [단독]
// If the badge doesn't exist yet (new banners), clone it from a donor node.
// ---------------------------------------------------------------------------

async function showExclusiveBadgeFrames(params) {
  const { pageName, badgeFrameName, donorBadgeId } = params || {};
  const badgeNames = ["Frame 38", "Frame 37"]; // accept either
  const primaryBadge = badgeFrameName || "Frame 38";
  ...
  // Load donor badge node for cloning into banners that lack one
  let donorNode = null;
  if (donorBadgeId) {
    donorNode = await figma.getNodeByIdAsync(donorBadgeId);
  }
  if (!donorNode) {
    // Auto-find a donor: look for any Frame 38 / Frame 37 on the page
    const findDonor = (node) => {
      if (badgeNames.includes(node.name) && !donorNode) {
        donorNode = node;
      }
      if ("children" in node) {
        for (const c of node.children) { if (!donorNode) findDonor(c); }
      }
    };
    for (const child of page.children) { if (!donorNode) findDonor(child); }
  }

  const shown = [];
  const cloned = [];
  const bannerRe = /^\d{2}_/;
  const exclusiveRe = /\[.*?단독.*?\]|단독/;

  for (const bannerFrame of page.children) {
    const t = bannerFrame.type;
    if (t !== "FRAME" && t !== "COMPONENT" && t !== "INSTANCE") continue;
    const name = String(bannerFrame.name).replace(/^#\s*/, "").trim();
    if (!bannerRe.test(name)) continue;

    // Check if exclusive: look in productTitle text
    let isExclusive = false;
    let productInfoNode = null;
    const findExclusive = (node) => {
      if (node.type === "TEXT" && node.name === "productTitle") {
        if (exclusiveRe.test(node.characters || "")) isExclusive = true;
      }
      if (node.name === "productInfo" || node.name === "productBox") {
        productInfoNode = node;
      }
      if ("children" in node) {
        for (const c of node.children) findExclusive(c);
      }
    };
    findExclusive(bannerFrame);
    if (!isExclusive) continue;

    // Try to find existing badge in this banner
    let badgeNode = null;
    const findBadge = (node) => {
      if (!badgeNode && badgeNames.includes(node.name)) badgeNode = node;
      if ("children" in node) {
        for (const c of node.children) { if (!badgeNode) findBadge(c); }
      }
    };
    findBadge(bannerFrame);

    if (badgeNode) {
      // Badge exists — just show it
      try { badgeNode.visible = true; } catch (_) {}
      shown.push({ bannerId: bannerFrame.id, bannerName: bannerFrame.name, badgeId: badgeNode.id, action: "shown" });
    } else if (donorNode) {
      // Clone the donor badge and insert into productInfo (or productBox)
      try {
        const clone = donorNode.clone();
        clone.name = primaryBadge;
        clone.visible = true;
        const target = productInfoNode || bannerFrame;
        target.appendChild(clone);
        cloned.push({ bannerId: bannerFrame.id, bannerName: bannerFrame.name, clonedId: clone.id, action: "cloned" });
      } catch (e) {
        shown.push({ bannerId: bannerFrame.id, bannerName: bannerFrame.name, error: String(e && e.message ? e.message : e), action: "error" });
      }
    }
  }

  const total = shown.length + cloned.length;
  figma.notify(`단독 배지 ${total}개 처리 (표시:${shown.length} 복제:${cloned.length})`);
  return {
    page: page.name,
...
// ---------------------------------------------------------------------------
// Apply linear gradient overlay to productImage in necklace/pendant banners
// ---------------------------------------------------------------------------

async function applyNecklaceImageGradient(params) {
  const { pageName, stops, transform } = params || {};
  let page = figma.currentPage;
  if (pageName) {
    await figma.loadAllPagesAsync();
    const p = figma.root.children.find((n) => n.name === pageName);
  ...
  // F5F5F5 (0.961) 100%→0% default, left-to-right
  const gradStops = stops && stops.length >= 2 ? stops : [
    { position: 0.05, color: { r: 0.961, g: 0.961, b: 0.961, a: 1.0 } },
    { position: 0.25, color: { r: 0.961, g: 0.961, b: 0.961, a: 0.0 } },
  ];
  // Default: horizontal left→right gradient transform
  const gradTransform = transform || [[1, 0, 0], [0, 1, 0]];

  const necklaceRe = /목걸이|펜던트/;
  const bannerRe = /^\d{2}_/;
  const applied = [];
  ...
  for (const bannerFrame of page.children) {
    const t = bannerFrame.type;
    if (t !== "FRAME" && t !== "COMPONENT" && t !== "INSTANCE") continue;
    const frameName = String(bannerFrame.name).replace(/^#\s*/, "").trim();
    if (!bannerRe.test(frameName)) continue;

    // Check if this is a necklace/pendant product
    let isNecklace = necklaceRe.test(frameName);
    if (!isNecklace) {
      const checkText = (node) => {
        if (node.type === "TEXT" && node.name === "productTitle") {
          if (necklaceRe.test(node.characters || "")) isNecklace = true;
        }
        if ("children" in node) {
      ...
      checkText(bannerFrame);
    }
    if (!isNecklace) continue;

    // Find productImage node and apply gradient
    const applyGrad = (node) => {
      if (node.name === "productImage" && (node.type === "FRAME" || node.type === "RECTANGLE")) {
        try {
          const existingFills = Array.isArray(node.fills) ? [...node.fills] : [];
          const gradFill = {
            type: "GRADIENT_LINEAR",
            gradientTransform: gradTransform,
            gradientStops: gradStops.map((s) => ({
              position: s.position,
              color: { r: s.color.r, g: s.color.g, b: s.color.b, a: s.color.a !== undefined ? s.color.a : 1 },
            })),
            opacity: 1,
          };
          // Append gradient on top (last = topmost in Figma)
          node.fills = [...existingFills, gradFill];
          applied.push({ bannerId: bannerFrame.id, bannerName: bannerFrame.name, nodeId: node.id });
        } catch (_) {}
      }
      if ("children" in node) {
        for (const c of node.children) applyGrad(c);
      }
    };
    applyGrad(bannerFrame);
  }

  figma.notify(`목걸이/펜던트 그라디언트 ${applied.length}개 적용`);
  return { page: page.name, appliedCount: applied.length, nodes: applied };
}

/** 옵션 행까지 복제했던 예전 배너(프레임명에 " · " 포함)만 삭제한다. */
async function deleteOptionSuffixBannerFrames(params) {
  const { pageName } = params || {};
  const targetPage =
    pageName && String(pageName).trim()
      ? String(pageName).trim()
      : "lotteOnLiveProductPage";
  await figma.loadAllPagesAsync();
  const p = figma.root.children.find((n) => n.name === targetPage);
  if (!p || p.type !== "PAGE") {
    throw new Error("Page not found: " + targetPage);
  }
  if (figma.currentPage.id !== p.id) await figma.setCurrentPageAsync(p);
  await p.loadAsync();
  const bannerRe = /^\d{2}_/;
  const removed = [];
  const snapshot = [...p.children];
  for (const child of snapshot) {
    const t = child.type;
    if (t !== "FRAME" && t !== "COMPONENT" && t !== "INSTANCE") continue;
    const n = String(child.name).replace(/^#\s*/, "").trim();
    if (!bannerRe.test(n)) continue;
    if (!n.includes(" · ")) continue;
    removed.push({ id: child.id, name: child.name });
    child.remove();
  }
  figma.notify("옵션 접미 배너 " + removed.length + "개 삭제");
  return { page: p.name, deleted: removed.length, frames: removed };
}

// ---------------------------------------------------------------------------
// Update only final-sale price TEXT inside each live banner frame (by layer name)
// ---------------------------------------------------------------------------

function normalizeBannerFrameName(name) {
  return String(name || "")
    .replace(/[\u00a0\u200b-\u200d\ufeff]/g, "")
    .replace(/\s*\/\s*/g, " · ")
    .replace(/[·・∙⋅]/g, "·")
...
async function findBannerFrameInDocument(frameName) {
  await figma.loadAllPagesAsync();
  for (const p of figma.root.children) {
    if (p.type !== "PAGE") continue;
    await p.loadAsync();
    const f = findBannerFrameByName(p, frameName);
    if (f) return { frame: f, page: p };
  }
  return { frame: null, page: null };
}

function findBannerFrameByName(page, frameName) {
  const exact = String(frameName);
  let node = page.findOne((n) => n.name === exact);
  if (node) return node;
  const want = normalizeBannerFrameName(exact);
  let found = null;
  const walk = (n) => {
    if (found) return;
    const t = n.type;
    if (
      ...
      normalizeBannerFrameName(n.name) === want
    ) {
      found = n;
      return;
    }
...
/** 최종가 TEXT: (1) 이름===textLayerName (2) finalPrice 프레임 자식 중 최종판매가 제외 (3) 이름이 숫자+원 */
function collectFinalPriceTextTargets(bannerFrame, layerName) {
  const targets = [];
  const walkByName = (node) => {
    if (node.type === "TEXT" && node.name === layerName) targets.push(node);
    if ("children" in node) {
      for (const c of node.children) walkByName(c);
    }
  };
  walkByName(bannerFrame);
  if (targets.length > 0) return targets;

  const walkFinalPriceFrame = (node) => {
    if (node.type === "FRAME" && node.name === "finalPrice") {
      for (const c of node.children || []) {
        if (c.type === "TEXT" && c.name !== "최종판매가") targets.push(c);
      }
      return;
    }
    if ("children" in node) {
      for (const c of node.children) walkFinalPriceFrame(c);
    }
  };
  walkFinalPriceFrame(bannerFrame);
  if (targets.length > 0) return targets;

  const re = /^\d[\d,]*원$/;
  const walkLegacy = (node) => {
    if (node.type === "TEXT" && re.test(String(node.name).trim())) targets.push(node);
    if ("children" in node) {
      for (const c of node.children) walkLegacy(c);
    }
  };
  walkLegacy(bannerFrame);
  return targets;
}

/** 이름이 `01_…` 또는 `# 01_…` 형태인 배너 프레임을 모아 위→아래(y), 왼→오른(x) 정렬 */
function collectBannerFramesInLayoutOrder(pg) {
  const bannerRe = /^\d{2}_/;
  const out = [];
  function walk(node) {
    const t = node.type;
    if (t === "FRAME" || t === "COMPONENT" || t === "INSTANCE") {
      const head = String(node.name)
        .replace(/^#\s*/, "")
        .trim();
...
async function updateLiveBannerFinalPrices(params) {
  const { pageName, textLayerName, items, fontMap, layoutOrder } = params || {};
  const layerName = textLayerName || "233,000원";
  if (!Array.isArray(items) || items.length === 0) {
  ...
  async function applyOneFrame(frame, it, meta) {
    const targets = collectFinalPriceTextTargets(frame, layerName);
    if (targets.length === 0) {
      results.push({
    ...
    const str = String(it.finalPrice);
    let ok = true;
    let errMsg = "";
    for (const t of targets) {
      try {
        ...
        const fm =
          (fontMap && fontMap[layerName]) ||
          (fontMap && fontMap["233,000원"]) ||
          (fontMap && fontMap["66,000원"]);
        if (fm && fm.family && fm.style) {
          await figma.loadFontAsync({ family: fm.family, style: fm.style });
          t.fontName = { family: fm.family, style: fm.style };
        }
        t.characters = str;
      } catch (e) {
        ok = false;
  ...
  if (layoutOrder) {
    const frames = collectBannerFramesInLayoutOrder(page);
    const n = Math.min(frames.length, items.length);
    for (let i = 0; i < n; i++) {
      const it = items[i];
      if (it.finalPrice === undefined || it.finalPrice === null) {
        results.push({
          frameName: frames[i].name,
          layoutIndex: i,
          status: "skip",
          reason: "missing finalPrice",
        });
        continue;
      }
      await applyOneFrame(frames[i], it, { layoutIndex: i });
    }
    for (let i = n; i < items.length; i++) {
      results.push({
        ...
        layoutIndex: i,
        status: "skip",
        reason: "no banner frame at this index (layout order)",
      });
    }
  } else {
    for (const it of items) {
      if (!it.frameName || it.finalPrice === undefined || it.finalPrice === null) {
        results.push({
          frameName: it.frameName,
          status: "skip",
          reason: "missing frameName or finalPrice",
        });
        continue;
      }
      let frame = findBannerFrameByName(page, it.frameName);
      if (!frame) {
        const { frame: f2 } = await findBannerFrameInDocument(it.frameName);
        frame = f2;
      }
      if (!frame) {
        results.push({ frameName: it.frameName, status: "missing_frame" });
        continue;
      }
      await applyOneFrame(frame, it, { layoutIndex: null });
    }
  }
  ...
  if (
    ...
    results.every(function (r) {
      return r.status === "missing_frame";
    })
  ) {
    await figma.loadAllPagesAsync();
    hint =
      "배너 프레임 이름이 한 페이지에도 없습니다. PAGE 목록: " +
      figma.root.children
        .filter((n) => n.type === "PAGE")
        .map((n) => n.name)
        .join(", ");
  }
  if (layoutOrder && okCount === 0 && items.length && results.length) {
    const anyNoText = results.some(function (r) {
      return r.status === "no_text_layer";
    });
    const allSkipLayout = results.every(function (r) {
      return r.status === "skip" && r.reason === "no banner frame at this index (layout order)";
    });
    if (results[0] && results[0].reason === "no banner frame at this index (layout order)" && allSkipLayout) {
      hint =
        "layoutOrder: 이름이 01_… 형태인 배너 프레임을 페이지에서 찾지 못했습니다. (SECTION 안에만 있으면 구조 확인)";
    } else if (anyNoText && !hint) {
      hint =
        "finalPrice TEXT를 찾지 못했습니다. finalPrice 그룹·레이어 이름(66,000원 등) 확인.";
    }
  }
  const payload = {
    page: page.name,
...
// ---------------------------------------------------------------------------
// Composite: clone a template banner, arrange in a grid, and sync data
// ---------------------------------------------------------------------------

/** productBox fill: naverLiveProductPage #F5F5F5, kakao 등 그 외 #FAFAFA. */
function applyProductBoxFill(node, pageName) {
  const isNaver = pageName === "naverLiveProductPage";
  const rgb = isNaver
    ? { r: 245 / 255, g: 245 / 255, b: 245 / 255 }
    : { r: 250 / 255, g: 250 / 255, b: 250 / 255 };
  if (node.type === "FRAME" && node.name === "productBox") {
    try {
      node.fills = [{ type: "SOLID", color: rgb }];
    } catch (_) {}
  }
  if ("children" in node) {
    for (const c of node.children) applyProductBoxFill(c, pageName);
  }
}

async function generateProductBanners(params) {
  const {
    templateNodeId,
    pageName,
    grid,
  ...
  if (!templateNodeId) throw new Error("Missing templateNodeId");
  ...
  const template = await figma.getNodeByIdAsync(templateNodeId);
  if (!template) throw new Error("Template not found: " + templateNodeId);

  const { cols, cellWidth, cellHeight, gapX, gapY, originX, originY } = grid;
  const gridPos = (i) => ({
    x: originX + (i % cols) * (cellWidth + gapX),
    y: originY + Math.floor(i / cols) * (cellHeight + gapY),
  });
  ...
  if (fontMap) {
    for (const k of Object.keys(fontMap)) {
      const f = fontMap[k];
      if (f && f.family && f.style) fontKeys.add(f.family + "|" + f.style);
    }
  }
  // Collect fonts from dualProduct items too
  for (const it of items) {
    if (it.dualProduct && it.dualProduct.fontMap) {
      for (const [, f] of Object.entries(it.dualProduct.fontMap)) {
        if (f && f.family && f.style) fontKeys.add(f.family + "|" + f.style);
      }
    }
  }
  ...
  for (const key of fontKeys) {
    ...
    try {
      await figma.loadFontAsync({ family, style });
      fontLoadResults[key] = "ok";
    } catch (e) {
      fontLoadResults[key] = "err:" + String(e && e.message ? e.message : e);
    }
  }

  // Template cache: key = nodeId → cloned template node (used for per-item templateNodeId)
  const templateCache = {};
  const getTemplate = async (nodeId) => {
    if (!templateCache[nodeId]) {
      templateCache[nodeId] = await figma.getNodeByIdAsync(nodeId);
    }
    return templateCache[nodeId];
  };
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // Support per-item templateNodeId for dual-product frames (예비 2)
    const itemTemplate = it.templateNodeId
      ? await getTemplate(it.templateNodeId)
      : template;
    if (!itemTemplate) throw new Error("Template not found: " + (it.templateNodeId || templateNodeId));

    if (it.frameName && existing[it.frameName]) {
      frame = await figma.getNodeByIdAsync(existing[it.frameName]);
    }
    if (!frame && it.frameName) {
      frame = findBannerFrameByName(page, it.frameName);
    }
    if (!frame) {
      frame = itemTemplate.clone();
      page.appendChild(frame);
      if (it.frameName) frame.name = it.frameName;
      created = true;
    }
    ...
    const textByName = {};
    const textByParentName = {};
    const imageByName = {};
    const traverse = (node) => {
      if (node.type === "TEXT") {
        if (!textByName[node.name]) textByName[node.name] = [];
        textByName[node.name].push(node);
        const parent = node.parent;
        if (parent && parent.name) {
          if (!textByParentName[parent.name]) textByParentName[parent.name] = [];
          textByParentName[parent.name].push(node);
        }
      }
      if (node.type === "FRAME" || node.type === "RECTANGLE") {
        if (!imageByName[node.name]) imageByName[node.name] = [];
        imageByName[node.name].push(node);
      }
      if ("children" in node) {
    ...
    traverse(frame);
    applyProductBoxFill(frame, page.name);

    const resolveTextTargets = (key) => {
      if (textByName[key] && textByName[key].length) return textByName[key];
      if (textByParentName[key] && textByParentName[key].length)
        return textByParentName[key];
      return [];
    };

    if (it.texts) {
      for (const [nodeName, chars] of Object.entries(it.texts)) {
        const targets = resolveTextTargets(nodeName);
        for (const t of targets) {
          const font =
            (fontMap && fontMap[nodeName]) ||
            (t.fontName && typeof t.fontName === "object" ? t.fontName : null);
          if (font && font.family && font.style) {
            if (fontLoadResults[font.family + "|" + font.style] !== "ok") {
              try {
                await figma.loadFontAsync({
                  family: font.family,
                  style: font.style,
                });
                fontLoadResults[font.family + "|" + font.style] = "ok";
              } catch (e) {
                fontLoadResults[font.family + "|" + font.style] =
    ...
    if (fontMap) {
      for (const [nodeName, font] of Object.entries(fontMap)) {
        if (!font || !font.family || !font.style) continue;
        const targets = resolveTextTargets(nodeName);
        for (const t of targets) {
          if (fontLoadResults[font.family + "|" + font.style] !== "ok") continue;
          try {
            t.fontName = { family: font.family, style: font.style };
          } catch (_) {}
        }
    ...
    const imgNodes = imageByName["productImage"] || [];

    if (it.dualProduct) {
      // ── 듀얼 프레임 처리 (예비 2 / 예비용_슈특3) ─────────────────
      // 사이드 루트: priceLeft 자손을 포함하는 priceRight/priceSection 중
      // 다른 후보를 포함하지 않는 노드 2개를 x 좌표 순으로 정렬 (left/right)
      const hasDescendantNamed = (node, name) => {
        if (!("children" in node)) return false;
        for (const c of node.children) {
          if (c.name === name) return true;
          if (hasDescendantNamed(c, name)) return true;
        }
        return false;
      };
      ...
      const collectSideCandidates = (node) => {
        if (
          node.type === "FRAME" &&
          (node.name === "priceRight" || node.name === "priceSection") &&
          hasDescendantNamed(node, "priceLeft")
        ) {
          sideCandidates.push(node);
        }
        if ("children" in node) {
          for (const c of node.children) collectSideCandidates(c);
        }
      };
      collectSideCandidates(frame);
      const priceRightNodes = sideCandidates.filter(
        (c) => !sideCandidates.some((o) => o !== c && containsNode(c, o))
      );
      priceRightNodes.sort((a, b) => {
        const ax = a.absoluteBoundingBox ? a.absoluteBoundingBox.x : (a.x || 0);
        const bx = b.absoluteBoundingBox ? b.absoluteBoundingBox.x : (b.x || 0);
        return ax - bx;
      });

      // productImage 노드 2개를 x 좌표 순으로 정렬
      const imgNodesSorted = [...imgNodes].sort((a, b) => {
        const ax = a.absoluteBoundingBox ? a.absoluteBoundingBox.x : (a.x || 0);
        const bx = b.absoluteBoundingBox ? b.absoluteBoundingBox.x : (b.x || 0);
        return ax - bx;
      });

      const ensureFont = async (fontObj) => {
        if (!fontObj || !fontObj.family || !fontObj.style) return;
        const key = fontObj.family + "|" + fontObj.style;
        if (fontLoadResults[key] !== "ok") {
          try { await figma.loadFontAsync(fontObj); fontLoadResults[key] = "ok"; } catch (_) {}
        }
      };

      const setTextInNode = async (node, value, fontHint) => {
        if (!node || node.type !== "TEXT") return;
        const fn = fontHint || (node.fontName && typeof node.fontName === "object" ? node.fontName : null);
        if (fn) await ensureFont(fn);
        else {
          try { if (node.fontName) await figma.loadFontAsync(node.fontName); } catch (_) {}
        }
        try { node.characters = String(value); } catch (_) {}
        if (fn && fontLoadResults[fn.family + "|" + fn.style] === "ok") {
          try { node.fontName = fn; } catch (_) {}
        }
      };

      const sides = ["left", "right"];
      for (let si = 0; si < sides.length; si++) {
        const sideKey = sides[si];
        const sideData = it.dualProduct[sideKey];
        if (!sideData) continue;

        const priceRight = priceRightNodes[si];
        const imgNode = imgNodesSorted[si];

        // 이미지 설정
        if (imgNode) {
          if (sideData.imageUrl) {
            imagePlan.push({ node: imgNode, url: sideData.imageUrl, gradient: null });
          } else {
            try { const fb = await getFallbackImageHash(); await applyImageFillToNode(imgNode, fb, "FILL"); } catch (_) {}
          }
        }

        if (priceRight) {
          // priceLeft 안의 TEXT 노드들 순서대로: [0]=variantLabel, [1]=originalPrice, [2]=discountPrice
          const priceLeftFrames = [];
          const discountRateTexts = [];
          const finalPriceTexts = [];

          const collectSideNodes = (node) => {
            if (node.name === "priceLeft" && "children" in node) {
              priceLeftFrames.push(node);
            }
            if (node.name === "discountRate" && "children" in node) {
              // discountRate 프레임 내 첫 번째 TEXT
              const t = node.children.find(c => c.type === "TEXT");
              if (t) discountRateTexts.push(t);
            }
            if (node.name === "finalPrice" && "children" in node) {
              // finalPrice 프레임 내 TEXT들: [0]=라벨, [1]=숫자
              const texts = node.children.filter(c => c.type === "TEXT");
              if (texts.length > 0) finalPriceTexts.push(...texts);
            }
            if ("children" in node && node.name !== "priceLeft" && node.name !== "discountRate" && node.name !== "finalPrice") {
              for (const c of node.children) collectSideNodes(c);
            }
          };
          collectSideNodes(priceRight);

          if (priceLeftFrames.length > 0) {
            const plTexts = priceLeftFrames[0].children.filter(c => c.type === "TEXT");
            // 텍스트 3개: [variantLabel, originalPrice, discountPrice] (예비 2)
            // 텍스트 2개: [originalPrice, discountPrice] (예비용_슈특3)
            const off = plTexts.length >= 3 ? 1 : 0;
            if (off === 1 && plTexts[0] && sideData.variantLabel != null)
              await setTextInNode(plTexts[0], sideData.variantLabel,
                { family: "Pretendard", style: "SemiBold" });
            if (plTexts[off] && sideData.originalPrice != null)
              await setTextInNode(plTexts[off], sideData.originalPrice,
                { family: "Pretendard", style: "Regular" });
            if (plTexts[off + 1] && sideData.discountPrice != null)
              await setTextInNode(plTexts[off + 1], sideData.discountPrice,
                { family: "Pretendard", style: "Bold" });
          }
          if (discountRateTexts[0] && sideData.discountRate != null)
            await setTextInNode(discountRateTexts[0], sideData.discountRate,
              { family: "Pretendard", style: "Bold" });
          // finalPrice 프레임 텍스트: 마지막이 금액 ([라벨, 금액] 또는 [금액])
          if (finalPriceTexts.length > 0 && sideData.finalPrice != null)
            await setTextInNode(finalPriceTexts[finalPriceTexts.length - 1], sideData.finalPrice,
              { family: "Pretendard", style: "ExtraBold" });
        }
      }
    } else {
      // ── 단독(예비 1) 프레임 처리 (기존 로직) ─────────────────────
      if (it.imageUrl && imgNodes.length > 0) {
        for (const n of imgNodes) {
          imagePlan.push({ node: n, url: it.imageUrl, gradient: it.imageGradient || null });
        }
      } else if (!it.imageUrl && imgNodes.length > 0) {
        try {
          const fb = await getFallbackImageHash();
          for (const n of imgNodes) await applyImageFillToNode(n, fb, "FILL");
        } catch (_) {}
      }
    }

    // showChildFrames: 지정한 이름의 자식 프레임을 visible=true 로 설정
    if (Array.isArray(it.showChildFrames) && it.showChildFrames.length > 0) {
      const showNames = new Set(it.showChildFrames.map((s) => String(s)));
      const setVisible = (node) => {
        if ("name" in node && showNames.has(String(node.name))) {
          try { node.visible = true; } catch (_) {}
        }
        if ("children" in node) {
          for (const c of node.children) setVisible(c);
        }
      };
      setVisible(frame);
    }
    ...
    results.push({
      ...
      x: frame.x,
      y: frame.y,
      textTargets: Object.keys(textByName),
    });
  }

  const batch = Math.max(1, Math.min(5, Number(imageBatchSize) || 3));
  const imgResults = [];
  for (let i = 0; i < imagePlan.length; i += batch) {
    const slice = imagePlan.slice(i, i + batch);
    const settled = await Promise.all(
      slice.map(async (p) => {
        try {
          const loaded = await loadImageFromUrl(p.url);
          await applyImageFillToNode(p.node, loaded.hash, "FILL");
          // 이미지 그라디언트 오버레이 적용 (목걸이 등)
          if (p.gradient) {
            try {
              const g = p.gradient;
              ...
              // 0도 = 왼쪽→오른쪽: transform for LINEAR
              const gradFill = {
                type: "GRADIENT_LINEAR",
              ...
              const existingFills = Array.isArray(p.node.fills) ? [...p.node.fills] : [];
              p.node.fills = [...existingFills, gradFill];
            } catch (_) {}
          }
          return {
            id: p.node.id,
          ...
          try {
            const fb = await getFallbackImageHash();
            await applyImageFillToNode(p.node, fb, "FILL");
            return {
              id: p.node.id,
              url: p.url,
              status: "fallback",
    ...
    imgResults.push(...settled);
    figma.notify(
      "banner image " + Math.min(i + batch, imagePlan.length) + "/" + imagePlan.length
    );
  }
  ...
  return {
    ...
    imageOk: imgResults.filter((r) => r.status === "ok").length,
    imageFallback: imgResults.filter((r) => r.status === "fallback").length,
    imageError: imgResults.filter((r) => r.status === "error").length,
    cacheSize: __imageUrlHashCache.size,
    frames: results,
  };
}
